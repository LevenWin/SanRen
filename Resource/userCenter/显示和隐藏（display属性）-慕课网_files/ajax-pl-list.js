define(function(require,exports){
    ...
    var bindDOMFuns = {
        praiseClick : function($this){
            $praise = $this.find("span");
            ...
            $.ajax({
                ...
                success : function (json){
                    if(json.result == 0){
                        ...
                        $this.addClass('on');
                        $this.find('i').addClass('on icon-thumb-revert');
                        $this.attr('title', '取消赞');
                    }
                }
            });
        },
        praiseCancel : function($this){
            $praise = $this.find("span");
            $.ajax({
                ...
                success : function (json){
                    if(json.result == 0){
                        ...
                        $this.removeClass('on');
                        $this.find('i').attr('class', 'icon-thumb-revert');
                        $this.attr('title', '赞');
                    }
                }
            });
    ...
    function pl(options){
        ...
        $(document).on("click",".js-pl-praise",function(e){
            ...
            if(_this.hasClass('on')){
                bindDOMFuns.praiseCancel(_this)
            }else{
                bindDOMFuns.praiseClick(_this);
            }
        });

define(function(require, exports, module) {
	...
	//var phpRunURI = 'http://cr.fw.imooc.com/';
    //var dockerRunURI = 'http://10.100.135.41/run/viewresult?file=';
	var dockerRunURI = 'http://cr.fw.imooc.com/run/viewresult?file=';
	...
	CodeEditor.prototype = {
		...
		runCode: function(isCommit,callback) {
			if(callback){
				...
				if(!isContent) {
					...
					//console.log(runURI + data.file)
						...
						if(data.cat == 'java'){
							...
							exe_uri = dockerRunURI;
						/*if(course_id == 26 || course_id == 116){
						exe_uri = "http://codeserver.imooc.com/run/viewresult?file=";
						}*/
						}else if(data.cat == 'python'){
							exe_uri = dockerRunURI;
						}else if(data.cat == 'c'){
							exe_uri = dockerRunURI;
						}else if(data.cat == 'cpp'){
							exe_uri = dockerRunURI;
						}else if(data.cat == 'oc'){
							exe_uri = dockerRunURI;
						}else if(data.cat == 'golang'){
							exe_uri = dockerRunURI;
						}else{
							exe_uri = runURI;
						}
						that.fileurl=data.file;
						if(that.callback) {
							that.callback();
							that.callback=null
						}
			...
			if(that.files[0].lang=='php' && that.files.length == 1 && isCommit){ //php
			    ...
			    $.post(saveURI, { lang: 'php',cid : course_id, code:that.files[0].content, sid:sid, filename:filename }, function(data) {
			...
			else if(that.files[0].lang=='python' && that.files.length == 1 && isCommit){ //python
				$.post(saveURI, { lang: 'python',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
					data.cat = 'python';
				 	frameShow(data);
				//html += '检测结果：'+data.result+'</p>';
				});
			}
			else if(that.files[0].lang=='golang' && that.files.length == 1 && isCommit){ //python
				$.post(saveURI, { lang: 'golang',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
					data.cat = 'golang';
				 	frameShow(data);
				});
			}
			else if(that.files[0].lang=='ruby' && that.files.length == 1 && isCommit){ //python
				$.post(saveURI, { lang: 'ruby',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
				 frameShow(data);
				//html += '检测结果：'+data.result+'</p>';
				});
			}
			else if(that.files[0].lang=='sass' && that.files.length == 1 && isCommit){ //python
				$.post(saveURI, { lang: 'sass',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
				 frameShow(data);
				//html += '检测结果：'+data.result+'</p>';
				});
			}
			else if(that.files[0].lang=='c' && that.files.length == 1 && isCommit){ //c
				 $.post(saveURI, { lang: 'c',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
				 	data.cat = 'c';
				 	frameShow(data)
				 });
			 }
			else if(that.files[0].lang=='oc' && that.files.length == 1 && isCommit){ //c
				 $.post(saveURI, { lang: 'oc',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
				 	data.cat = 'oc';
				 	frameShow(data)
				 });
			 }
			else if(that.files[0].lang=='oc' && that.files.length > 1 && isCommit){//多个c文件
				var xdata,
					suc = 0,
					len = that.files.length;
				$.each(that.files, function(i, v) {
					$.post(saveURI, { lang: 'oc',cid : course_id, code:v.content , sid:sid, filename:v.filename}, function(data) {
						suc++;
						if(i==0){
							xdata = data;
						}
			...
			else if(that.files[0].lang=='cpp' && that.files.length == 1 && isCommit){ //c
				 $.post(saveURI, { lang: 'cpp',cid : course_id, code:that.files[0].content , sid:sid, filename:that.files[0].filename}, function(data) {
				 	data.cat = 'cpp';
					frameShow(data)
				 });
			 }
			else if(that.files[0].lang=='c' && that.files.length > 1 && isCommit){//多个c文件
				var xdata,
					suc = 0,
					len = that.files.length;
				$.each(that.files, function(i, v) {
					$.post(saveURI, { lang: 'c',cid : course_id, code:v.content , sid:sid, filename:v.filename}, function(data) {
						suc++;
						if(i==0){
							xdata = data;
						}
				...
				$.post(saveURI, { lang: 'javascript',cid : course_id, code:that.files[0].content }, function(data) {
				  	frameShow(data)
					//html += '检测结果：'+data.result+'</p>';
				});
		   	}
				...
				$.each(that.files, function(i, v) {
					if (v.lang == 'php' && isCommit) {
						...
						//html += '检测结果：'+data.result+'</p>';
                                                $.post(saveURI, { lang: 'php',cid : course_id, filename:v.filename, sid:sid, code:v.content }, function(data) {
                                                    ...
                                                    if(count == that.files.length){//dockerRunURI
                                                        ...
                                                        for (var i = 0; i < res_save_arr.length; i++) {
                                                            if (res_save_arr[i].file.indexOf("index") != -1) { 
                                                                /*if(course_id == 26 || course_id == 116){
                                                                    dockerRunURI = "http://codeserver.imooc.com/run/viewresult?file=";
                                                                }*/
					...
					else if(v.lang=='python' && isCommit){
						$.post(saveURI, { lang: 'python',cid : course_id, code:v.content }, function(data) {
			...
			else if(that.files[0].lang=='java' && that.files.length == 1 && isCommit){ //c
				$.post(saveURI, { lang: 'java',cid : course_id, code:that.files[0].content, filename:that.files[0].filename, sid:sid }, function(data) {
					data.cat = 'java';
					frameShow(data)
				});
			}else if(that.files[0].lang == 'java' && that.files.length > 1 && isCommit){
				$.each(that.files ,function(i,a){
					$.post(saveURI, { lang: 'java',cid : course_id, code:a.content, filename:a.filename, sid:sid }, function(data) {
						data.cat = 'java';
						if(i == 0){
							frameShow(data)
						}
			...
			else if(that.files[0].lang=='csharp' && that.files.length == 1 && isCommit){ //c 
				$.post(saveURI, { lang: 'csharp',cid : course_id, code:that.files[0].content, filename:that.files[0].filename, sid:sid }, function(data) {
					data.cat = 'csharp';
					frameShow(data)
				});
			}
			    ...
			    frameShow(that.files[0].content, 1);
					...
					$.post(saveURI, { lang: 'php',cid : course_id, code:that.files[1].content, sid:sid, filename:filename }, function(data) {
						if(data && data.result_type){
							$('#J_Console').attr('src', 'http://php.mukewang.com/'+sid+'/'+filename+'?rnd='+Math.random());
						}
			...
			else if(that.files.length == 1 && isCommit){ //single file : go
		...
		commit: function(lang,callback) {
			...
			return function(lang,callback){
				var that= this;
				this.runCode(1,function(){
					
					var params = {};
	                var filelen = that.files.length;
					params.cid = course_id;
					params.mid = pageInfo.mid;
					params.eid = pageInfo.eid;
					params.step = pageInfo.step;
					params.lang = lang;
					params.file = that.fileurl;
	                	...
	                	success:function(data) {
							...
							if(data.result === -200) {
								...
								if(pageInfo.next_mid==0){
									...
									if($("#J_CodeLang").data("regular")=="1"){
										tip = sucMsg[Math.floor(Math.random() * msgLength)];
									}else{
										tip="该节编程练习不验证代码，你可以各种尝试，或继续学习"
									}

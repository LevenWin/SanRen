...
$(function(){
	...
	var initEditor =function(){
		...
		window.editor=UE.getEditor("discuss-editor",{initialFrameHeight:150,autoHeight: true,autoFloatEnabled:false,autoClearinitialContent:true,initialStyle:'p{line-height:1.5em;font-size:13px;color:#444}'});
    ...
    function commentSubmit(checked){
		...
		if($('.pl-input-btm .verify-code input').length!==0){
    ...
    $('.js-btn-follow').click(function(e){
    	...
    	$.ajax({
			type: "post",
			//url: url,
			url:"/u/"+OP_CONFIG.userInfo.uid+"/course/"+$this.attr('data-id'),
			dataType:"json",
			// data: {
	...
	$('.section-list .notes').on('click',function(){
		...
		window.notepop = $.dialog($("#course-note-tpl").html(),{
			title:"笔记"
		})
    ...
    $('.section-list .wiki').on('click',function(){
		...
		window.wikipop = $.dialog($("#course-wiki-tpl").html(),{
			title:"搜索WIKI"
		})
	...
	$('.section-list .question').on('click',function(){
		...
		$.ajax({
			...
			success: function(res) {
				...
				if(res.code==1) {
					//第一、二次免费发问题
					window.qapop = $.dialog($("#course-qa-tpl").html(),{
						title:"提问",
						callback:function(){

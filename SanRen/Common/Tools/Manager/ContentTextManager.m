...
- (NSMutableAttributedString *)highlightText:(NSMutableAttributedString *)coloredString{
    ...
    for (NSString *str in stringArr) {
        if ([str containsString:@"@"]) {
            UIColor *highlightColor = UIColorFromRGB(0x297bc1);
            [coloredString addAttribute:(NSString*)kCTForegroundColorAttributeName
                                  value:(id)highlightColor.CGColor range:[string rangeOfString:[NSString stringWithFormat:@"@%@",[str componentsSeparatedByString:@"@"].lastObject]]];
            [self.ranges addObject:NSStringFromRange([string rangeOfString:str])];
            
            
        }
    }
    //#
    __block BOOL jinTag=NO;
    __block NSInteger location=0;
    __block NSMutableAttributedString *copyAttrString=[[NSMutableAttributedString alloc]initWithAttributedString:coloredString] ;
    ...
    for (NSInteger i=0 ;i<string.length;i++) {
        ...
        if ([substring isEqualToString:@"#"]) {
            if (jinTag) {
                UIColor *highlightColor = UIColorFromRGB(0x297bc1);
                [copyAttrString addAttribute:(NSString*)kCTForegroundColorAttributeName
                                       value:(id)highlightColor.CGColor range:NSMakeRange(location, i-location+1)];
                [self.ranges addObject:NSStringFromRange(NSMakeRange(location, i-location+1))];

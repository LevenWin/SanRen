...
-(void)processPoint:(CGPoint )point{
    ...
    [self heightAll:begin];
    
    if (self.contentSelect&&!begin) {
        self.contentSelect(nil,NSMakeRange(0, 0));
    }

    
}
-(void)heightAll:(BOOL)re{
    ...
    [attributedText addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:textRange];
    if (re) {
        [attributedText addAttribute:NSBackgroundColorAttributeName value:[UIColor colorWithRed:0.8265 green:0.8265 blue:0.8265 alpha:1.0] range:NSMakeRange(0, self.text.length)];
    }else{
        
        
        [attributedText addAttribute:NSBackgroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(0, self.text.length)];

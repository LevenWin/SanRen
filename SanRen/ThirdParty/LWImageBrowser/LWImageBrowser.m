        ...
        default: {
            ...
            [UIView animateWithDuration:0.25f
                             ...
                             animations:^{
                                 ...
                                 weakSelf.currentImageItem.imageView.alpha=0;
                                 
                             } completion:^(BOOL finished) {
                                 [weakSelf dismissViewControllerAnimated:NO completion:^{
                                     if ([self.delegate respondsToSelector:@selector(imageBrowserDidFinish)]
                                         && [self.delegate conformsToProtocol:@protocol(LWImageBrowserDelegate)]) {
                                         [self.delegate imageBrowserDidFinish];
                                     }
...
- (void)_tapMenuButton {
    LWActionSheetView* actionSheet = [[LWActionSheetView alloc] initTilesArray:@[@"保存到本地",@"取消"] delegate:self];
    [actionSheet show];
}
...
- (void)lwActionSheet:(LWActionSheetView *)actionSheet didSelectedButtonWithIndex:(NSInteger)index {
    NSLog(@"%ld",index);
    if (index==0) {
        LWImageBrowserModel *model=self.imageModels[self.currentIndex];
        UIImageView *imageV =[[UIImageView alloc] init];
    
    [imageV sd_setImageWithURL:model.HDURL completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
{
    if (!error) {
//        [MyUIHelper showSuccessMsg:@"成功保存到相册"];
       
    }else
    {
//        [MyUIHelper showSuccessMsg:[error description]];
